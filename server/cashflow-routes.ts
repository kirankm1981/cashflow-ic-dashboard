...
async function reprocessTbData(): Promise<number> {
  ...
  for (let offset = 0; offset < total; offset += BATCH) {
    ...
    for (const row of batch) {
      ...
      if (changed) {
        updatePromises.push(
          db.update(cashflowTbData).set(newFields).where(eq(cashflowTbData.id, row.id))
        );
      }
    }
...
let mappingCache: {
  ...
  entities: { mappings: any[]; companyKeys: Set<string>; excludedKeys: Set<string> } | null;
  lastLoaded: number;
...
async function getEntityCache() {
  ...
  const [entityMappings, excluded] = await Promise.all([
    db.select().from(cashflowMappingEntities),
    db.select().from(cashflowExcludedEntities),
  ]);
  const excludedKeys = new Set(excluded.map(e => normalizeText(e.company)).filter(Boolean));
  const companyKeys = new Set(
    entityMappings.map(e => normalizeText(e.companyNameErp || e.companyName || ""))
      .filter(k => k && !excludedKeys.has(k))
  );
  mappingCache.entities = { mappings: entityMappings, companyKeys, excludedKeys };
  return mappingCache.entities;
}
...
export function registerCashflowRoutes(app: Express) {
  ...
  async function processCashflowTbInBackground(fileId: number, filePath: string, label: string) {
    try {
      await db.update(cashflowTbFiles).set({ status: "parsing", statusMessage: "Parsing file..." }).where(eq(cashflowTbFiles.id, fileId));
      ...
      await db.update(cashflowTbFiles).set({
        enterprise, period: periodRaw, totalRecords: validRows.length,
        status: "inserting", statusMessage: `Parsed ${validRows.length} rows. Inserting...`,
      }).where(eq(cashflowTbFiles.id, fileId));
      ...
      for (const e of entityCached.mappings) {
        ...
        const buKey = normalizeText(e.businessUnit || "");
        if (buKey) {
          buMap.set(buKey, { projectName: e.projectName, entityStatus: e.entityStatus });
        }
      }
      ...
      for (let i = 0; i < validRows.length; i += BATCH_SIZE) {
        ...
        if (inserted % 2000 < BATCH_SIZE) {
          await db.update(cashflowTbFiles).set({ statusMessage: `Inserting rows... ${inserted} / ${validRows.length}` }).where(eq(cashflowTbFiles.id, fileId));
        }
      }
      ...
      logUpload("CASHFLOW", "TB-UPLOAD OK", `File ${fileId} — ${inserted} rows stored`);
    } catch (error: any) {
      logUpload("CASHFLOW", "TB-UPLOAD BG ERROR", `File ${fileId}: ${error.message}`);
      console.error(`[cashflow] TB background processing error for file ${fileId}:`, error);
      cleanupFile(filePath);
      await db.update(cashflowTbFiles).set({
        status: "error", statusMessage: error.message || "Processing failed",
      }).where(eq(cashflowTbFiles.id, fileId)).catch(() => {});
    }
  }
  ...
  app.post("/api/cashflow/upload-tb", upload.single("file"), async (req, res) => {
    try {
      ...
      logUpload("CASHFLOW", "TB-UPLOAD START", `File: ${req.file.originalname} (${(req.file.size / 1024).toFixed(0)} KB)`);
      ...
      processCashflowTbInBackground(tbFile.id, req.file.path, label);
    } catch (error: any) {
      logUpload("CASHFLOW", "TB-UPLOAD ERROR", `${req.file?.originalname}: ${error.message}`);
      if (req.file?.path) cleanupFile(req.file.path);
      console.error("Cashflow TB upload error:", error);
      const detail = error.message || String(error);
      res.status(error.status || 500).json({ message: `Internal server error: ${detail}` });
    }
  });
  ...
  app.post("/api/cashflow/upload-mapping", upload.single("file"), async (req, res) => {
    ...
    try {
      if (!req.file) return res.status(400).json({ message: "No file uploaded" });
      logUpload("CASHFLOW", "MAPPING-UPLOAD START", `File: ${req.file.originalname} (${(req.file.size / 1024).toFixed(0)} KB)`);
      ...
      if (groupingsSheet) {
        ...
        const grpHeaderRow = findHeaderRow(rows);
        const rawHeaders = (rows[grpHeaderRow] || []).map((h: any) => String(h || "").trim());
        ...
        const dataRows = rows.slice(grpHeaderRow + 1).filter(r => !isBlankRow(r) && String(r[ahCol] || "").trim() !== "");
        const BATCH = 500;
      ...
      if (entitySheet) {
        ...
        const entHeaderRow = findHeaderRow(rows);
        const rawHeaders = (rows[entHeaderRow] || []).map((h: any) => String(h || "").trim());
        const headers = rawHeaders.map(h => h.toLowerCase().replace(/[^a-z0-9]/g, ""));
        const colIdx = (...aliases: string[]) => {
          for (const alias of aliases) {
            const norm = alias.toLowerCase().replace(/[^a-z0-9]/g, "");
        ...
        console.log(`[Entity] Header row: ${entHeaderRow}, Headers detected: ${rawHeaders.join(", ")}`);
        ...
        const dataRows = rows.slice(entHeaderRow + 1).filter(r => !isBlankRow(r) && String(r[firstCol] || "").trim() !== "");
        const BATCH = 500;
        for (let i = 0; i < dataRows.length; i += BATCH) {
          const batch = dataRows.slice(i, i + BATCH);
          const values = batch.map(r => {
      ...
      if (pastLossesSheet) {
        ...
        const dataRows = rows.slice(headerIdx + 1).filter(r => !isBlankRow(r) && String(r[coCol] || "").trim() !== "");
        const BATCH = 200;
        for (let i = 0; i < dataRows.length; i += BATCH) {
          const batch = dataRows.slice(i, i + BATCH);
          const values = batch.map(r => ({
      ...
      let excludedInserted = 0;
      const unmappedSheet = findSheet("Unmapped_Entities") || findSheet("Unmapped Entities");
      if (unmappedSheet) {
        await db.delete(cashflowExcludedEntities);
        const rows: any[][] = sheetData[unmappedSheet];
        const umHeaderRow = findHeaderRow(rows);
        const rawHeaders = (rows[umHeaderRow] || []).map((h: any) => String(h || "").trim());
        const headers = rawHeaders.map(h => h.toLowerCase().replace(/[^a-z0-9]/g, ""));
        const colIdx = (...aliases: string[]) => {
          for (const alias of aliases) {
            const norm = alias.toLowerCase().replace(/[^a-z0-9]/g, "");
        ...
        const iComp = colIdx("company", "companyname");
        const firstCol = iComp >= 0 ? iComp : 0;
        console.log(`[Unmapped_Entities] Header row: ${umHeaderRow}, Headers: ${rawHeaders.join(", ")}`);
        const dataRows = rows.slice(umHeaderRow + 1).filter(r => !isBlankRow(r) && String(r[firstCol] || "").trim() !== "");
        const seen = new Set<string>();
        const toInsert: { company: string; reason: string }[] = [];
        for (const r of dataRows) {
          const company = String(r[firstCol] || "").trim();
          const key = company.toLowerCase();
          if (company && !seen.has(key)) {
            seen.add(key);
            toInsert.push({ company, reason: "" });
          }
        }
        const BATCH = 500;
        for (let i = 0; i < toInsert.length; i += BATCH) {
          await db.insert(cashflowExcludedEntities).values(toInsert.slice(i, i + BATCH));
          excludedInserted += Math.min(BATCH, toInsert.length - i);
        }
        console.log(`[Unmapped_Entities] Inserted ${excludedInserted} excluded entities`);
      }

      invalidateMappingCache();

      logUpload("CASHFLOW", "MAPPING-UPLOAD OK", `${req.file.originalname} — ${groupingsInserted} groupings, ${entitiesInserted} entities, ${pastLossesInserted} past losses, ${excludedInserted} excluded`);

      let reprocessed = 0;
      const tbCount = await db.select({ count: sql<number>`count(*)` }).from(cashflowTbData);
      if (Number(tbCount[0]?.count || 0) > 0) {
        console.log("Auto-reprocessing TB data with updated mappings...");
        reprocessed = await reprocessTbData();
        console.log(`Auto-reprocess complete: ${reprocessed} records updated`);
      }

      cleanupFile(req.file.path);
      res.json({
        message: `Mapping uploaded: ${groupingsInserted} groupings, ${entitiesInserted} entities, ${pastLossesInserted} past losses, ${excludedInserted} excluded. ${reprocessed > 0 ? `Reprocessed ${reprocessed} TB records.` : ""}`,
        groupingsInserted,
        entitiesInserted,
        pastLossesInserted,
        excludedInserted,
      ...
      logUpload("CASHFLOW", "MAPPING-UPLOAD ERROR", `${req.file?.originalname}: ${error.message}`);
      if (req.file?.path) cleanupFile(req.file.path);
  ...
  app.get("/api/cashflow/tb-files", async (_req, res) => {
    try {
      ...
      const message = `Internal server error: ${error.message || String(error)}`;
  ...
  app.get("/api/cashflow/mapping-summary", async (_req, res) => {
    try {
      ...
      const distinctEntities = new Set(
        entityCached.mappings
          .map(e => normalizeText(e.companyNameErp || e.companyName || ""))
          .filter(Boolean)
      );
      const excluded = await db.select({ count: sql<number>`count(*)` }).from(cashflowExcludedEntities);
      res.json({
        hasMapping: groupingCached.raw.length > 0 || entityCached.mappings.length > 0,
        groupings: groupingCached.raw.length,
        entities: distinctEntities.size,
        pastLosses: pastLosses.length,
        excludedEntities: Number(excluded[0]?.count || 0),
      });
    } catch (error: any) {
      console.error("[cashflow] Route error:", error);
      const message = `Internal server error: ${error.message || String(error)}`;
  ...
  app.get("/api/cashflow/summary", async (_req, res) => {
    try {
      ...
      const entityCached = await getEntityCache();
      const excludedKeys = entityCached.excludedKeys;
      const tbCompanies = await db.selectDistinct({ company: cashflowTbData.company }).from(cashflowTbData);
      const allTbCompanyNames = tbCompanies
        .map(r => (r.company || "").trim())
        .filter(name => name !== "");
      const activeEntityCount = allTbCompanyNames
        .filter(name => !excludedKeys.has(normalizeText(name)))
        .length;

      const excludedCompanySet = new Set(
        allTbCompanyNames.filter(name => excludedKeys.has(normalizeText(name)))
      );
      const activeRecordCount = excludedCompanySet.size > 0
        ? Number((await db.select({ count: sql<number>`count(*)` }).from(cashflowTbData)
            .where(sql`${cashflowTbData.company} NOT IN (${sql.join([...excludedCompanySet].map(c => sql`${c}`), sql`, `)})`)
          )[0]?.count || 0)
        : Number(dataCount[0]?.count || 0);
  ...
  app.post("/api/cashflow/reprocess", async (_req, res) => {
    try {
      invalidateMappingCache();
      const updated = await reprocessTbData();
  ...
  app.get("/api/cashflow/download-mapped-tb", async (_req, res) => {
    try {
      ...
      const mappedRows = allRows.filter(r => !excludedKeys.has(normalizeText(r.company || "")));
  ...
  app.get("/api/cashflow/dashboard-data", async (_req, res) => {
    try {
      ...
      const validCompanies = entityCached.mappings
        ...
        .filter(c => c && !entityCached.excludedKeys.has(normalizeText(c)));
  ...
  app.get("/api/cashflow/download-detailed", async (_req, res) => {
    try {
      ...
      const tbAgg = await db.select({
        ...
        .groupBy(cashflowTbData.company, cashflowTbData.businessUnit, cashflowTbData.projectName, cashflowTbData.entityStatus, cashflowTbData.cashflow, cashflowTbData.cfHead);
  ...
  app.get("/api/cashflow/download-unmapped", async (_req, res) => {
    try {
      ...
      const entityCachedDl = await getEntityCache();
      const allEntityAgg = await db.select({
        company: cashflowTbData.company,
        businessUnit: cashflowTbData.businessUnit,
        accountHead: cashflowTbData.accountHead,
        netClosingBalance: sql<number>`sum(${cashflowTbData.netClosingBalance})`,
      ...
      const unmappedEntityAgg = allEntityAgg.filter(r => {
        const key = normalizeText(r.company || "");
        return !entityCachedDl.companyKeys.has(key) && !entityCachedDl.excludedKeys.has(key);
      });
  ...
  app.get("/api/cashflow/unmapped-items", async (_req, res) => {
    try {
      ...
      for (const e of entityCached.mappings) {
        const key = normalizeText(e.companyNameErp || e.companyName || "");
        if (key) entityMap.set(key, e);
      }

      const allEntityGroups = await db.select({
        company: cashflowTbData.company,
        businessUnit: cashflowTbData.businessUnit,
        netClosingBalance: sql<number>`sum(${cashflowTbData.netClosingBalance})`,
        count: sql<number>`count(*)`,
      ...
      const unmappedEntities = allEntityGroups.filter(r => {
        const key = normalizeText(r.company || "");
        return !entityCached.companyKeys.has(key) && !entityCached.excludedKeys.has(key);
      });
  ...
  app.get("/api/cashflow/download-mapping", async (_req, res) => {
    try {
      ...
      const excluded = await db.select().from(cashflowExcludedEntities).orderBy(asc(cashflowExcludedEntities.company));
      if (excluded.length > 0) {
        const umSheet = XLSX.utils.json_to_sheet(excluded.map(e => ({
          "Company": e.company || "",
        })));
        umSheet["!cols"] = [{ wch: 50 }];
        XLSX.utils.book_append_sheet(wb, umSheet, "Unmapped_Entities");
      }
  ...
  app.get("/api/cashflow/excluded-entities", requireAuth, async (_req, res) => {
    try {
      const excluded = await db.select().from(cashflowExcludedEntities);
      const allCompanies = await db.selectDistinct({ company: cashflowTbData.company }).from(cashflowTbData);
      res.json({
        excluded: excluded.map(e => ({ id: e.id, company: e.company, reason: e.reason || "" })),
        availableCompanies: allCompanies.map(c => c.company).filter(Boolean).sort(),
      });
    } catch (error: any) {
      console.error("[cashflow] Route error:", error);
      const detail = error.message || String(error);
  ...
  app.post("/api/cashflow/excluded-entities", requireAuth, async (req, res) => {
    try {
      const { companies } = req.body as { companies: Array<{ company: string; reason?: string }> };
      if (!companies || !Array.isArray(companies) || companies.length === 0) {
        return res.status(400).json({ message: "No companies provided" });
      }
      const existing = await db.select().from(cashflowExcludedEntities);
      const existingKeys = new Set(existing.map(e => normalizeText(e.company)));
      const toInsert = companies
        .filter(c => c.company && !existingKeys.has(normalizeText(c.company)))
        .map(c => ({ company: c.company.trim(), reason: c.reason?.trim() || null }));
      if (toInsert.length > 0) {
        await db.insert(cashflowExcludedEntities).values(toInsert);
      }
      invalidateMappingCache();
      res.json({ added: toInsert.length, total: existing.length + toInsert.length });
    } catch (error: any) {
      console.error("[cashflow] Route error:", error);
      const detail = error.message || String(error);
      res.status(500).json({ message: `Internal server error: ${detail}` });
  ...
  app.delete("/api/cashflow/excluded-entities/:id", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) return res.status(400).json({ message: "Invalid ID" });
      await db.delete(cashflowExcludedEntities).where(eq(cashflowExcludedEntities.id, id));
      invalidateMappingCache();
  ...
  app.post("/api/cashflow/upload-exclusion-list", requireAuth, upload.single("file"), async (req, res) => {
    try {
      if (!req.file) return res.status(400).json({ message: "No file uploaded" });
      logUpload("CASHFLOW", "EXCLUSION-UPLOAD START", `File: ${req.file.originalname} (${(req.file.size / 1024).toFixed(0)} KB)`);
      const parsed = await parseFileInWorker(req.file.path, req.file.originalname);
      cleanupFile(req.file.path);
      if (!parsed || parsed.length === 0) {
        return res.status(400).json({ message: "File contains no data" });
      }
      const companies: Array<{ company: string; reason: string | null }> = [];
      for (const row of parsed) {
        const company = String(row["Company"] || row["company"] || row["Company Name"] || row["Entity"] || Object.values(row)[0] || "").trim();
        const reason = String(row["Reason"] || row["reason"] || row["Remarks"] || row["remarks"] || "").trim() || null;
        if (company) {
          companies.push({ company, reason });
        }
      }
      if (companies.length === 0) {
        return res.status(400).json({ message: "No company names found in file" });
      }
      await db.delete(cashflowExcludedEntities);
      const BATCH = 500;
      for (let i = 0; i < companies.length; i += BATCH) {
        await db.insert(cashflowExcludedEntities).values(companies.slice(i, i + BATCH));
      }
      invalidateMappingCache();
      logUpload("CASHFLOW", "EXCLUSION-UPLOAD OK", `${req.file.originalname} — ${companies.length} companies imported`);
      res.json({ imported: companies.length });
    } catch (error: any) {
      console.error("[cashflow] Route error:", error);
      logUpload("CASHFLOW", "EXCLUSION-UPLOAD ERROR", `${req.file?.originalname}: ${error.message}`);
      if (req.file?.path) cleanupFile(req.file.path);
      const detail = error.message || String(error);
      res.status(500).json({ message: `Internal server error: ${detail}` });
    }
  ...
  app.delete("/api/cashflow/clear-exclusions", requireAuth, async (_req, res) => {
    try {
      await db.delete(cashflowExcludedEntities);
      invalidateMappingCache();
