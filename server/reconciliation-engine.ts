...
export async function runReconciliation(): Promise<{
  ...
  if (reconRunning) {
    throw Object.assign(new Error("Reconciliation is already in progress. Please wait for it to complete."), { status: 409 });
  }
  reconRunning = true;
  try {
    return await runReconciliationInternal();
  } finally {
    reconRunning = false;
  }
}

async function runReconciliationInternal(): Promise<{
  totalMatched: number;
  ruleResults: { rule: string; matched: number; classification: string }[];
}> {
...
function buildNetOffPairs(lines: SummarizedLine[]): NetOffPairGroup[] {
  ...
  for (const t of lines) {
    ...
    const txnType = (t.icTxnType || "").trim();
    const dirKey = `${normCompany}||${normCounter}||${txnType}`;
  ...
  for (const [dirKey, dirLines] of directionMap) {
    ...
    const [normA, normB] = parts;
    const txnType = parts[2] || "";
    const reverseKey = `${normB}||${normA}||${txnType}`;
...
async function reversalMatch(
  ...
  for (const t of allUnmatched) {
    ...
    const txnType = (t.icTxnType || "").trim();
    const key = `${normalizeCompanyName(t.company)}||${normalizeCompanyName(t.counterParty)}||${txnType}`;
  ...
  for (const [groupKey, lines] of byCompanyCP) {
    ...
    const [normCompany, normCounter] = parts;
