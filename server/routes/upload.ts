...
export function registerUploadRoutes(app: Express) {
  ...
  app.post("/api/upload/sheet-names", requireWriteAccess, upload.single("file"), async (req, res) => {
    try {
      ...
      cleanupFile(req.file.path);
      logUpload("IC-RECON", "SHEET-NAMES OK", `Sheets found: ${sheetNames.join(", ")}`);
      res.json({ sheetNames });
    } catch (error: any) {
      logUpload("IC-RECON", "SHEET-NAMES ERROR", `${req.file?.originalname}: ${error.message}`);
  ...
  app.post("/api/upload/preview-headers", requireWriteAccess, upload.single("file"), async (req, res) => {
    try {
      ...
      cleanupFile(req.file.path);
      logUpload("IC-RECON", "PREVIEW-HEADERS OK", `${req.file.originalname} — headers parsed`);
      res.json(result);
    } catch (error: any) {
      logUpload("IC-RECON", "PREVIEW-HEADERS ERROR", `${req.file?.originalname}: ${error.message}`);
      if (req.file?.path) cleanupFile(req.file.path);
      const isOperational = error.status && error.status < 500;
      const message = isOperational ? (error.message || String(error)) : `Internal server error: ${error.message || String(error)}`;
      res.status(error.status || 500).json({ message });
  ...
  app.post("/api/upload", requireWriteAccess, upload.single("file"), async (req, res) => {
    try {
      if (!req.file) return res.status(400).json({ message: "No file uploaded" });
      logUpload("IC-RECON", "UPLOAD START", `File: ${req.file.originalname} (${(req.file.size / 1024).toFixed(0)} KB)`);
      ...
      const filteredRecords = records.filter((r: any) => Object.values(r).some((v: any) => String(v ?? "").trim() !== ""));

      const txns: InsertTransaction[] = filteredRecords.map((r: any) => {
      ...
      logUpload("IC-RECON", "UPLOAD OK", `${req.file.originalname} — ${inserted.length} records, ${insertedLines.length} summarized lines, batch ${batchId}`);
      res.json({
        batchId,
        totalRecords: inserted.length,
      ...
      if (req.file?.path) cleanupFile(req.file.path);
      logUpload("IC-RECON", "UPLOAD ERROR", `${req.file?.originalname}: ${error.message}`);
      console.error("Upload error:", error);
      res.status(error.status || 500).json({ message: error.message || String(error) });
    }
  });

  app.post("/api/transactions/clear", requireWriteAccess, async (_req, res) => {
    try {
  ...
  app.post("/api/upload/reconciliation", requireWriteAccess, upload.single("file"), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ message: "No file uploaded" });
      }
      logUpload("IC-RECON", "RECON-UPLOAD START", `File: ${req.file.originalname} (${(req.file.size / 1024).toFixed(0)} KB)`);
      ...
      logUpload("IC-RECON", "RECON-UPLOAD OK", `${req.file.originalname} — ${groupsCreated} groups, ${totalMatched} matched`);
      res.json({
      ...
      if (req.file?.path) cleanupFile(req.file.path);
      logUpload("IC-RECON", "RECON-UPLOAD ERROR", `${req.file?.originalname}: ${error.message}`);
      const isOperational = error.status && error.status < 500;
      const message = isOperational ? (error.message || String(error)) : `Internal server error: ${error.message || String(error)}`;
      res.status(error.status || 500).json({ message });
    }
