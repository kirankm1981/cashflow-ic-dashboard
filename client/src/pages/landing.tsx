...
function MisCardStats() {
  ...
  const activeRecords = data.activeRecordCount || 0;
  ...
  return (
    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs" data-testid="stats-mis">
      ...
      {activeRecords > 0 && <span><span className="text-muted-foreground">Records: </span><span className="font-semibold">{activeRecords.toLocaleString()}</span></span>}
...
const statsComponents: Record<string, () => JSX.Element | null> = {
