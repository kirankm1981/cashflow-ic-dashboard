...
export function D2PlWip({ rows, formatConfig }: Props) {
  ...
  const wipByComponent = useMemo(() => {
    ...
    for (const r of wipRows) {
      ...
      map[comp] = (map[comp] || 0) + Math.abs(r.closingNet);
    }
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    const sheet = buildPLSheet(plSums, PL_ORDER, PL_SIGNS, calculatedLabels);
                    downloadExcel([sheet], `PL_Statement_${new Date().toISOString().slice(0, 10)}.xlsx`);
                  }}
                  disabled={plRows.length === 0}
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      disabled={wipRows.length === 0}
                      data-testid="button-download-wip"
                    >
                      <Download className="w-3.5 h-3.5 mr-1" />
                      Download
                    </Button>
