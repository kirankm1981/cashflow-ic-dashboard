      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm">Working Capital Breakdown</CardTitle>
            <Button
              variant="outline"
              size="sm"
              onClick={() => {
                const sheet = buildWorkingCapitalSheet(assetBuckets, liabilityBuckets);
                downloadExcel([sheet], `Working_Capital_${new Date().toISOString().slice(0, 10)}.xlsx`);
              }}
              disabled={wcRows.length === 0}
              data-testid="button-download-wc"
            >
