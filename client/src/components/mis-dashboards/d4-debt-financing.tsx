  ...
  const periodMonths = useMemo(() => {
    ...
    if (dateMatch) {
      ...
      const endDate = new Date(
        parseInt(dateMatch[6]), parseInt(dateMatch[5]) - 1, parseInt(dateMatch[4])
      );
    ...
    const shortMatch = period.match(/(\w{3})-(\w{3})\s+\d{4}/);
    if (shortMatch) {
      const m: Record<string, number> = {
        Jan:1, Feb:2, Mar:3, Apr:4, May:5, Jun:6,
        Jul:7, Aug:8, Sep:9, Oct:10, Nov:11, Dec:12
      };
      const start = m[shortMatch[1]] || 1;
      const end = m[shortMatch[2]] || 12;
