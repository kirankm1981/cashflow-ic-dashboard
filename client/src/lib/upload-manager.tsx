    ...
    try {
      ...
      const data = await xhrUpload("/api/recon/upload-gl", formData, (pct) => {
        updateNotification(notifId, {
          progress: pct,
          message: pct < 100 ? `Uploading file... ${pct}%` : "File accepted, processing in background...",
      ...
      updateNotification(notifId, {
        progress: 100,
        status: "success",
        message: `${data.fileName} — accepted. Parsing and saving rows in background.`,
      });
  ...
  const uploadTbFile = useCallback(async (file: File, label: string, slotId: string, periodStart?: string, periodEnd?: string) => {
    ...
    try {
      ...
      updateNotification(notifId, {
        progress: 100,
        status: "success",
        message: `${label} — Processing in background. Progress will update automatically.`,
      });
