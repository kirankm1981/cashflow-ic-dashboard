...
export const icMatrixTbData = pgTable("ic_matrix_tb_data", {
  ...
  icTxnType: text("ic_txn_type"),
  rptTxnType: text("rpt_txn_type"),
  rptGroup: text("rpt_group"),
  companyCode: text("company_code"),
  tbSource: text("tb_source"),
  currentNonCurrent: text("current_non_current"),
  fsGroup: text("fs_group"),
  fsHeading: text("fs_heading"),
});
...
export const icEntityList = pgTable("ic_entity_list", {
  id: serial("id").primaryKey(),
  shortName: text("short_name").notNull(),
  companyName: text("company_name"),
  companyNameErp: text("company_name_erp"),
  doiConversion: text("doi_conversion"),
...
export type IcEntityListEntry = typeof icEntityList.$inferSelect;

export const icRptAuditEntries = pgTable("ic_rpt_audit_entries", {
  id: serial("id").primaryKey(),
  entityCompanyCode: text("entity_company_code").notNull(),
  entityName: text("entity_name"),
  rptLedger: text("rpt_ledger").notNull(),
  rptCounterPartyCode: text("rpt_counter_party_code"),
  rptCounterPartyName: text("rpt_counter_party_name"),
  amount: doublePrecision("amount").notNull(),
  natureOfBalance: text("nature_of_balance"),
  narration: text("narration"),
  icTxnType: text("ic_txn_type"),
  rptTxnType: text("rpt_txn_type"),
  icType: text("ic_type"),
...
export const insertIcRptAuditEntrySchema = createInsertSchema(icRptAuditEntries).omit({
  id: true,
  createdAt: true,
});

export type InsertIcRptAuditEntry = z.infer<typeof insertIcRptAuditEntrySchema>;
export type IcRptAuditEntry = typeof icRptAuditEntries.$inferSelect;
...
export const cashflowTbData = pgTable("cashflow_tb_data", {
  ...
  cfStatementLine: text("cf_statement_line"),
  plCategory: text("pl_category"),
  plSign: doublePrecision("pl_sign").default(0),
  wipComponent: text("wip_component"),
  wcBucket: text("wc_bucket"),
...
export const cashflowExcludedEntities = pgTable("cashflow_excluded_entities", {
  id: serial("id").primaryKey(),
  company: text("company").notNull(),
  reason: text("reason"),
  excludedAt: text("excluded_at").$defaultFn(() => new Date().toISOString()),
}, (t) => [
  index("cf_excluded_company_idx").on(t.company),
]);

export type CashflowExcludedEntity = typeof cashflowExcludedEntities.$inferSelect;
